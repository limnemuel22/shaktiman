...
export class Global {
    ...
    getPurchases() {
        this.dbService.get("purchases").subscribe(data => {
    ...
    getItems() {
        this.dbService.get("items").subscribe(data => {
            if (data.status == undefined) {
                this.items = data;
                for (let i of this.items) {
                    i.price = Number(i.price).toLocaleString("en-us", { minimumFractionDigits: 2 }).toString();
                }
            }
        });
    }
    ...
    getUsers() {
        this.dbService.get("users", this.token).subscribe(data => {
            if (data.status == undefined) {
                this.users = data;
            }
        });
    ...
    getPayments() {
        this.dbService.get("payments", this.token).subscribe(data => {
            if (data.status == undefined) {
                this.payments = data;
                Object.entries(this.payments).forEach(([key, value]) => {
                    ...
                    this.payments[key].discountedPrice = Number(value['discountedPrice']).toLocaleString("en-us", { minimumFractionDigits: 2 });
    ...
    getAgents() {
        this.dbService.get("agents", this.token).subscribe(data => {
            if (data.status == undefined) {
                this.agents = data;
            }
        });
    ...
    getClients() {
        this.dbService.get("clients", this.token).subscribe(data => {
            if (data.status == undefined) {
                this.clients = data;
            }
        });
