...
export class UsereditComponent implements OnInit {
  ...
  createForm(){
    this.form = this.formbuilder.group({
      employeeName:['', Validators.compose([Validators.required,Validators.minLength(10),this.validateLetters])],
      birthday:['', Validators.compose([Validators.required,Validators.maxLength(10),this.validateDate])],
      position:['', Validators.compose([Validators.required,Validators.maxLength(20),this.validateLetters])],
      usertype:[''],
    });
  }
  ...
  getUser(){
    this.dbService.get("user",this.route.snapshot.params['id']).subscribe(data =>{
      this.model = data[0];
